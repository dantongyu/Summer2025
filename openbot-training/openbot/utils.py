...
def show_train_batch(image_batch, label_batch, fig_num=1):
    ...
    for n in range(15):
        ...
        plt.title(
            "Label: [%.2f %.2f]"
            % (float(label_batch[n][0]), float(label_batch[n][1]))
        )
        plt.axis("off")


def show_test_batch(image_batch, label_batch, pred_batch, fig_num=1):
    plt.figure(num=fig_num, figsize=(15, 10))
    for n in range(15):
        ax = plt.subplot(5, 3, n + 1)
        plt.imshow(image_batch[n])
        plt.title(
            "Label: [%.2f %.2f], Pred: [%.2f %.2f]"
            % (
...
def compare_tf_tflite(model, tflite_model, img=None, cmd=None, cmd_included=False):
    ...
    for input_detail in input_details:
        ...
        elif "cmd_input" in input_detail["name"] and cmd_included:
            if cmd is None:
