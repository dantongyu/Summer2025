...
def create_example(image, path, ctrl_cmd, ios=False):
    ...
    if not ios:
        try:
            feature = {
                ...
                "throttle": float_feature(float(ctrl_cmd[0]) / 255.0),
                "steer": float_feature(float(ctrl_cmd[1]) / 255.0),
            }
        ...
        feature = {
            "image": image_feature(image),
            "path": bytes_feature(path),
            "throttle": float_feature(float(ctrl_cmd[0])),
            "steer": float_feature(float(ctrl_cmd[1])),
        }
